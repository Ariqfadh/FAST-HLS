...
void generate_random_image(hls::stream<ap_uint<8>>& stream, 
                          ap_uint<11> height, ap_uint<11> width) {
...
bool validate_keypoint(const Keypoint& kp, ap_uint<11> height, ap_uint<11> width) {
    const int BORDER = 3;
    return (kp.x >= BORDER) && (kp.x < width - BORDER) &&
...
void run_test_case(ap_uint<11> height, ap_uint<11> width) {
    ...
    while (!keypoint_stream.empty()) {
        ...
        if (validate_keypoint(kp, height, width)) {
            ...
            std::cerr << "Invalid keypoint detected at (" 
                      << kp.x << ", " << kp.y << ")\n";
        }
    }

    std::cout << "Processing time: " << duration.count() << "ms\n";
    std::cout << "Detected keypoints: " << total_count 
              << " (Valid: " << valid_count << ")\n";
    std::cout << "------------------------------------------------\n";
}
...
int main() {
    ...
    // Stress test
    std::cout << "\nRunning Stress Test (10 consecutive images)...\n";
    ...
    for (int i = 0; i < 10; i++) {
        ...
        std::cout << "Stress test image " << i+1 
                  << " - Keypoints detected: " << output.size() << "\n";
    }

    auto end_stress = std::chrono::high_resolution_clock::now();
    auto duration_stress = std::chrono::duration_cast<std::chrono::milliseconds>(
        end_stress - start_stress);
